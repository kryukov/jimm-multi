...
public class FileSystemFileConnection implements FileConnection {
	...
	FileSystemFileConnection(String name, FileSystemConnectorImpl notifyClosed) throws IOException {
        ...
        // application path
        // "e:/jimm-multi/" is symlink to home
        if (fullPath.startsWith("e:/jimm-multi/")) {
            fullPath = fullPath.replaceFirst("e:/jimm-multi/", "~/");
        }
    ...
    private File getHome() {
        ...
        if (null != root) {
            home = new File(root, "jimm-multi");
            if (!home.exists()) home.mkdir();
            return home;
        }
        return null;
        //return new File(root, "Android/data/" + JimmActivity.PACKAGE_NAME + "/files/");
    }
    private File getExistHome(String rootName) {
        File root = getRoot(rootName);
        if (null != root) {
            File home = new File(root, "jimm-multi");
	...
	private static Vector<File> getLocalRoots() {
        ...
        try {
            File fsRoot = new File(System.getProperty("user.home") + "/.jimm/filesystem");
            if (!fsRoot.exists()) {
                if (!fsRoot.mkdirs()) {
