...
public class LGFile extends Vector {
    ...
    public LGFileSubset containsGroup(String key) {
        for (int i = 0; i < size(); i++) {
            if (super.get(i) instanceof LGFileSubset) {
                if (((LGFileSubset)get(i)).getId().equals(key)) {
                    return (LGFileSubset)get(i);
                }
            }
        }
        return null;
    ...
    private boolean optimizeR() {
        for (int i = 0; i < size(); i++) {
            for (int j = i + 1; j < size(); j++) {
                LGFileSubset si = (LGFileSubset)get(i);
                LGFileSubset sj = (LGFileSubset)get(j);
                if (sj.getId().equals(si.getId())) {
                    for (int k = 0; k < sj.size(); k++) {
                        si.addElement(sj.elementAt(k));
                    }
                    remove(sj);
                    return true;
                }
    ...
    public void optimize() {
        for (int i = 0; i < size(); i++) {
            if (((LGFileSubset) get(i)).getId().endsWith("_ELSE")) {
                return;
            }
        }
        while (optimizeR()) {};
    ...
    public void save(String path) throws Exception {
        ...
        for (int i = 0; i < size(); i++) {
            LGFileSubset subset = (LGFileSubset) get(i);
            if (!subset.isRemoved()) {
                if (subset.getId().startsWith("TAR_") && !subset.getId().endsWith("_ELSE")) {
                    ...
                    try {
                        print_end = !((LGFileSubset) get(i + 1)).getId().endsWith("_ELSE");
                    } catch (Exception e) {
                        print_end = true;
                    }
                } else if (subset.getId().startsWith("MOD_") && !subset.getId().endsWith("_ELSE")) {
                    ...
                    print_end = true;
                } else if (subset.getId().endsWith("_ELSE")) {
        ...
        while (file.ready()) {
            ...
            if (line.lastIndexOf("sijapp") != -1) {
                ...
                if (line.lastIndexOf("cond.else") != -1) {
                    ...
                    group = group + "_ELSE";
                } else if (line.lastIndexOf("cond.end") != -1) {
                    subset.setId(group);
                    temp.add(subset.getClone());
                    subset = new LGFileSubset();
                    group = null;
                }
            } else if (LGString.parseLine(line) != null) {
                if (group == null) {
