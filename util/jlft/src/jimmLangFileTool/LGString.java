...
public class LGString implements Cloneable {
    ...
    public static LGString parseLine(String line) {
        ...
        if (line.startsWith("//"))
            ...
            int indexStart = line.indexOf('"', indexEnd + 1);
            String[] keys = new String[2];
            keys[0] = line.substring(0, indexEnd);
            keys[1] = line.substring(indexStart + 1);
            String key = keys[0].substring(1, keys[0].length());
            String value = keys[1].substring(0, keys[1].length()-1);
